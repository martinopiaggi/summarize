...
This installs `fastapi`, `uvicorn`, and `python-multipart`. If startup fails with a pydantic version error, see [Errors and Troubleshooting](../usage/errors-and-troubleshooting).
...
| `speed` | number | Playback speed for audio preprocessing or visual-mode video (>0, ≤10) |
| `output_format` | string | `markdown`, `json`, or `html` |
| `visual` | boolean | Send video directly to a vision model |
| `use_proxy` | boolean | Route through the configured HTTP proxy |
