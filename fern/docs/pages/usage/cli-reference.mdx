...
<Tabs>
  ...
  <Tab title="Audio and Language">
    ...
    # Aggressive speed-up (any positive value)
    python -m summarizer --source "URL" --speed 5.0

    # Speed up video in visual mode (base64 path)
    python -m summarizer --source "URL" --provider nvidia --visual --speed 1.5
    ...
    # Select passages before summarization
...
| `--chunk-size` | Input text chunk size in characters (min 1000) | `120000` (from `summarizer.yaml`; code fallback `10000`) |
...
| `--speed` | Playback speed for audio preprocessing or visual-mode video. Reduces Groq cost (priced by duration) but may reduce accuracy. Values other than `1.0` skip YouTube captions. Any positive value. | `1.0` |
...
| `--parallel-calls` | Concurrent API requests | `10` (from `summarizer.yaml`; code fallback `30`) |
...
| `--use-jev-prefiltering` / `--no-use-jev-prefiltering` | Enable or disable transcript selection before summarization | From YAML (`false`) |
| `--jev-provider` | Existing OpenRouter or TypeSafe provider used for scoring | `openrouter` |
| `--jev-include` | Include only passages about this concept; blank means general relevance | `""` |
| `--jev-exclude` | Exclude matching passages; exclusions take priority | `""` |
| `--use-proxy` | Route supported requests through the configured HTTP proxy | From YAML |
...
Interactive docs are available at `http://localhost:8000/docs`. See the [HTTP API guide](api-reference/http-api) for endpoint reference and example requests.
