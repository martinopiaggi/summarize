...
<AccordionGroup>
  <Accordion title="Key defaults explained">
    ...
    - `use-jev-prefiltering: true` selects transcript passages before summarization. See [JEV Prefiltering](features/jev-prefiltering).
    - `visual-input-mode: url` on a provider sends YouTube URLs directly to the model without downloading (see [Visual Mode](features/visual-mode)).
