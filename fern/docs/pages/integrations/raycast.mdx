...
By default the API runs at `http://localhost:8000`. If you are developing from a cloned repo, use `pip install -e ".[server]"` inside a venv instead. See [Installation](../getting-started/installation) if startup fails with dependency errors.
