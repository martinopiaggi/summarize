...
More background on this project: [martino.im/Summarize](https://martino.im/Summarize.html).
...
<CardGroup cols={2}>
  ...
  <Card title="JEV Prefiltering" icon="fa-solid fa-filter">
    Optional selection of transcript passages before the summary model, including or excluding a particular concept
  </Card>
...
- **Transcript path** (default): downloads audio/video, transcribes with Whisper or captions, caches the complete transcript, optionally applies [JEV prefiltering](features/jev-prefiltering), then summarizes with an LLM.
