...
After running a source, switch between **OUTPUT** and **TRANSCRIPT**. The transcript tab shows the complete cached transcript when available (not the passages selected by JEV). **DOWNLOAD**, **COPY TO CLIPBOARD**, and **PUBLISH AND SHARE WITH TINYPASTE** use whichever tab is selected. If no transcript is available for a result, the transcript actions are hidden. Summaries restored from disk do not include their transcripts.
...
- **Use JEV prefiltering** — Reveals the scoring provider and **I want to include only…** / **I want to exclude…** fields. See [JEV Prefiltering](jev-prefiltering).
...
Copy the selected output or full cached transcript to the system clipboard with a single button. Works in both secure and non-secure browser contexts.
